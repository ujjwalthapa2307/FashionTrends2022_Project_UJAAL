...
df.to_csv(r't-shirts-flipkart-final-final.csv')
...
for row in dataframe1.itertuples():
    ...
    cursor.execute('''INSERT INTO T_shirts_Data (ID,URL,Brands,ITEM_NAME,DISCOUNTED_PRICE,MRP,STARS,No_Rating,No_Reviews,List_Reviews)VALUES (?,?,?,?,?,?,?,?,?)''',id,row.URL,row.BRAND,row.desc,row.DISCOUNTEDPRICE,row.MRP,stars,num_ratings,num_reviews,row.reviews)
